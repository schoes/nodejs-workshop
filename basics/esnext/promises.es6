...
Database.find(1).then((result) => {
    console.log(result);
});
Database.find(2).then((result) => {
    console.log(result);
});
Database.find(3).then((result) => {
    console.log(result);
});


//2. Do the same again, but let the calls run concurrently. Log the result
Promise.all([Database.find(1), Database.find(2), Database.find(3)]).then((result) => {
    console.log(result);
})